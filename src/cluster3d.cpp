...
template <typename PointT>
void ClusterPts<PointT>::clusterHelper(int ind, PtCdtr<PointT> cloud,
                                       std::vector<int> &cluster,
                                       KdTree *tree) {
  processed[ind] = true;
  cluster.push_back(ind);

  std::vector<int> nearest_point =
      tree->search(cloud->points[ind], distanceTol);
...
template <typename PointT>
std::vector<PtCdtr<PointT>> ClusterPts<PointT>::EuclidCluster(
    PtCdtr<PointT> cloud) {
